...
engine_path = "yolov7-w6-pose.engine"
image_path = "image/test.jpeg"
ctypes.CDLL(PLUGIN_LIBRARY)
TRT_LOGGER = trt.Logger()
# 57*1000+1
output_shape = (1, 57001)
...
def main():
    ...
    image_ = image.copy()
    input_x = np.array([transforms.ToTensor()(image).numpy()])
    ...
    no = result[0]
    result = result[1:]
    result = result.reshape((1000, 57))
    result = result[:int(no), :]

    output = torch.from_numpy(np.expand_dims(result, axis=0))
    ...
    cv2.imwrite("result.jpg", cv2.cvtColor(nimg, cv2.COLOR_BGR2RGB))
